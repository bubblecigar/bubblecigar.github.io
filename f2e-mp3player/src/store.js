...
export default new Vuex.Store({
  ...
  actions: {
    searchSongs({ }, { keywords, index }) {
      ...
      this.commit("SET_KEYWORDS", keywords);
      apiClient
    ...
    prevSong({ }) {
      if (this.state.songIndex === 0) {
        this.commit("SET_SONG_INDEX", this.state.songs.length - 1);
      } else {
        this.commit("MINUS_SONG_INDEX");
      }
    },
    randomSong({ }) {
      let newIndex = Math.floor(Math.random() * this.state.songs.length);
      this.commit("SET_SONG_INDEX", newIndex);
    }
  }
});
